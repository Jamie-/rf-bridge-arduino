...
String getRandString() {
  uint8_t r = random(0, 4); // Second value is exclusive
  dbg("Rand number is: " + String(r) + "\r\n");
  if (r == 0) {
    return String("North");
  } else if (r == 1) {
    return String("South");
...
void handleDataReq(ZBRxResponse rx) {
  ...
  if (rx.getDataLength() == 2) {
    ...
    if (device == sensorType && index == 0) {
      ...
      uint8_t payload[10] = {DATA_RESPONSE, (device << 4) + index};
      String outStr = getRandString();
      for (int i = 0; i < outStr.length(); i++) {
        payload[i + 2] = outStr[i];
      }
      sendResponse(rx, payload, 2 + outStr.length());
    } else {
      dbg("Device " + String(device) + ", and index " + String(index) + " requested, does not exist.\r\n");
