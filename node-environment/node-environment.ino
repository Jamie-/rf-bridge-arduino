...
String humidName = "Humidity";
uint8_t sensorType = INT_1B_OUTPUT;
...
uint8_t getTemperature() {
  ...
  if (isnan(t)) {
    dbg("Failed to read DHT sensor.\r\n");
    return 0;
  }
  dbg("Temperature: " + String(t) + "\r\n");
  return round(t);
}
...
void handleIOReq(ZBRxResponse rx) {
  ...
  uint8_t payload[2] = {IO_RESPONSE, (sensorType << 4) + 1}; // +1 to account for 2 payloads
  sendResponse(rx, payload, sizeof(payload));
  dbg("Done.\r\n");
}
...
void handleInfoReq(ZBRxResponse rx) {
  ...
  if (rx.getDataLength() == 2) {
    ...
    if (device == sensorType) {
      ...
      if (index == 0) {
        for (int i = 0; i < 2 + tempName.length(); i++) {
          payload[i + 2] = tempName[i];
        }
        sendResponse(rx, payload, 2 + tempName.length());
      } else if (index == 1) {
        for (int i = 0; i < 2 + humidName.length(); i++) {
          payload[i + 2] = humidName[i];
        }
        sendResponse(rx, payload, 2 + humidName.length());
      } else {
  ...
  if (rx.getDataLength() == 2) {
    ...
    if (device == sensorType) {
      ...
      if (index == 0) {
        ...
        payload[2] = getTemperature();
        sendResponse(rx, payload, sizeof(payload));
      } else if (index == 1) {
        // Get humidity data
        payload[2] = getHumidity();
        sendResponse(rx, payload, sizeof(payload));
      } else {
        dbg("Device " + String(device) + ", and index " + String(index) + " requested, does not exist.\r\n");
